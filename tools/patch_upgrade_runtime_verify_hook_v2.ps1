...
if($text -notmatch "_abp_runtime_verify_or_die"){ throw "Hook v1 introuvable (attendu avant upgrade v2)" }

# Replace the existing hook function block by a stronger one (bounded regex)
...
Write-Host "PATCH OK: runtime verify hook upgraded to v2 (STATE sig + STATE sha + release sha)"
