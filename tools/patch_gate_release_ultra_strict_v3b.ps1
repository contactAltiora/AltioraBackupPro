...
if($posBegin -lt 0 -or $posEnd -lt 0 -or $posEnd -le $posBegin){
  throw "Bloc gate BEGIN/END introuvable ou invalide dans release_build_and_backup.ps1"
}

# Supprimer l'ancien bloc (jusqu'à fin de ligne après END)
$afterEnd = $raw.IndexOf("`n", $posEnd)
if($afterEnd -lt 0){ $afterEnd = $posEnd + $end.Length } else { $afterEnd = $afterEnd + 1 }

# Nouveau bloc (sans here-string imbriquée)
$newBlockLines = @(
...
Write-Host "[PATCH] OK: Gate Ultra Strict remplacé (v3b)"
