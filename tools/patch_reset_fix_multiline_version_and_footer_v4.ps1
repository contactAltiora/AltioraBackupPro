...
# ------------------------------------------------------------
# 4) Fix EXISTING --version action='version' (MULTILINE) by inserting a proper python line:
#      version=VERSION_STR,
#    right before the closing ')'
#    IMPORTANT: we do NOT add any new --version.
# ------------------------------------------------------------
# We locate an add_argument(...) block that contains --version and action="version"
# and does NOT already contain "version=" inside that call.
$pat = "(?s)(?<call>\b\w+\.add_argument\(\s*.*?--version.*?action\s*=\s*['""]version['""].*?\n(?<ind>\s*)\)\s*)"
...
if($call -match "(?m)^\s*version\s*="){
  ...
  # inject as a new argument line BEFORE the closing paren line
  $call2 = [regex]::Replace(
    $call,
    "(?m)^(?<ind>\s*)\)\s*$",
    ($ind + "    version=VERSION_STR,`n" + $ind + ")")
  if($call2 -eq $call){ throw "Injection impossible (ligne de fermeture ')' introuvable dans le bloc --version)" }

  # replace in raw (by index)
  $raw = $raw.Substring(0,$m.Index) + $call2 + $raw.Substring($m.Index + $m.Length)
