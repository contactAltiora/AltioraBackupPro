...
# 0) Revenir à un altiora.py propre
git restore --source=HEAD --worktree --staged .\altiora.py | Out-Null
py -m py_compile .\altiora.py
if($LASTEXITCODE -ne 0){ throw "ERROR: altiora.py not clean after git restore HEAD" }

$py = Join-Path $root "tools\patch_cli_verify_handler_clean_v5c.py"
