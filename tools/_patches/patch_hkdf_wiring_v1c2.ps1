...
$py   = Join-Path $root "tools\patch_hkdf_wiring_v1c2.py"

if(-not (Test-Path $core)) { throw "ERROR: missing file: $core" }

# Write python patcher (no nested here-strings)
$pyLines = @(
...
"    ""        raise\n"",",
"]",
"replace_in_window(""salt = os.urandom(16)"", window=20, match_substr=""key = _derive_key("", new_block_lines=new_key_block)",
"",
...
"window_text = """".join(lines[idx:idx+40])",
"if '\"kdf\"' not in window_text:",
"    indent = lines[idx].split(""header:"")[0]",
"    insert_line = indent + ""    \""kdf\"": kdf_mode,\n""",
...
"print(""OK: HKDF wiring v1c2 applied to backup_core.py"")"
)

Set-Content -Path $py -Value $pyLines -Encoding UTF8

# Run patcher from project root
Set-Location $root
py -X utf8 $py
...
Write-Host "OK: py_compile backup_core.py + master_key.py"
