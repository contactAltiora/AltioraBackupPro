...
# 0) Sécurisation: remettre altiora.py EXACTEMENT comme HEAD (annule cmdfix v4 cassé)
git restore --source=HEAD --worktree --staged .\altiora.py | Out-Null

py -m py_compile .\altiora.py
if($LASTEXITCODE -ne 0){ throw "ERROR: altiora.py is not clean even after git restore HEAD" }

$cli  = Join-Path $root "altiora.py"
$py   = Join-Path $root "tools\patch_cli_verify_handler_clean_v5b.py"
...
bak = CLI.with_suffix(".py.bak_verifyhandler_v5b_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
shutil.copyfile(CLI, bak)

# 1) localiser le bloc verify existant
...
# 2) borne de fin : le prochain bloc restore (on remplace tout l'intérieur du verify)
...
body = (
    ...
    f'{indent}        return 0 if oks ok else 1\n'
)

# Petite sécurité: correction typo éventuelle (ci-dessus) -> on force la ligne correcte ensuite
body = body.replace("return 0 if R", "return 0 if ok else 1")

# Compléter le bloc (hors json)
body += (
    f'{indent}\n'
    f'{indent}    if ok:\n'
...
print(f"OK: verify handler replaced (v5b). Backup: {bak}")
'@
