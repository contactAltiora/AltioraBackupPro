...
# Keywords that open blocks (line ends with :)
...
for($i=0; $i -lt $lines.Count; $i++){
  ...
  # only operate inside run() body: (run+1 .. main-2)
  ...
  # must start with one of the known openers (simple "keyword " test)
  $isOpener = $false
  foreach($k in $kw){
    if($trim.StartsWith($k + " ") -or $trim -eq ($k + ":")){
      $isOpener = $true
      break
    }
  }
