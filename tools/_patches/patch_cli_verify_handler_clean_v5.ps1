...
$py   = Join-Path $root "tools\patch_cli_verify_handler_clean_v5.py"
...
bak = CLI.with_suffix(".py.bak_verifyhandler_v5_" + datetime.now().strftime("%Y%m%d_%H%M%S"))
...
# 2) borne de fin : le prochain "if args.command == "restore":" (on remplace tout l'intérieur)
...
# Nouveau bloc verify (indentation conservée)
body = (
...
print(f"OK: verify handler replaced (v5). Backup: {bak}")
'@
