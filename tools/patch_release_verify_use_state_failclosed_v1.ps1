...
# Locate injected block start/end
$begin = -1
$end   = -1
...
"",
"    # Fail-closed: if public key exists, enforcement is ON",
...
"        sys.exit(1)",
"",
"    # Prefer STATE.md version (source of truth)",
"    v = None",
"    if os.path.exists(state_path):",
"        try:",
"            txt = open(state_path, 'r', encoding='utf-8', errors='ignore').read()",
"            m = re.search(r'\\bv\\d+\\.\\d+\\.\\d+\\b', txt)",
"            if m:",
"                v = m.group(0)",
"        except Exception:",
"            v = None",
"",
"    # Determine target zip",
"    zip_path = None",
"    if v:",
...
"            zip_path = candidate",
"        else:",
"            # If STATE points to a version that is missing, fail-closed",
"            sys.exit(1)",
"    else:",
"        # Fallback: latest by mtime, but still fail-closed on missing signature",
...
Write-Host "release verify updated: STATE.md-based STRICT fail-closed"
