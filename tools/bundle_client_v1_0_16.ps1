...
$relZip = Join-Path $root "_out\releases\AltioraBackupPro_v1.0.16_release.zip"
$relSig = Join-Path $root "_out\releases\AltioraBackupPro_v1.0.16_release.zip.sig"
$relSha = Join-Path $root "_out\releases\AltioraBackupPro_v1.0.16_release.sha256"
...
# ---- Bundle locations
$bundleDir = Join-Path $root "_out\bundles\AltioraBackupPro_v1.0.16_client_bundle"
$outZip    = Join-Path $root "_out\bundles\AltioraBackupPro_v1.0.16_client_bundle.zip"
$outSha    = Join-Path $root "_out\bundles\AltioraBackupPro_v1.0.16_client_bundle.sha256"
...
Copy-Item -LiteralPath $relZip -Destination (Join-Path $bundleDir "_out\releases\AltioraBackupPro_v1.0.16_release.zip") -Force
Copy-Item -LiteralPath $relSig -Destination (Join-Path $bundleDir "_out\releases\AltioraBackupPro_v1.0.16_release.zip.sig") -Force
Copy-Item -LiteralPath $relSha -Destination (Join-Path $bundleDir "_out\releases\AltioraBackupPro_v1.0.16_release.sha256") -Force

# ---- README
@"
ALTIORA BACKUP PRO — Client Bundle v1.0.16

CONTENTS
- AltioraBackupPro.exe
- STATE.md + STATE.md.sig + STATE.md.sha256
- keys\altiora_public_key.pem
- _out\releases\AltioraBackupPro_v1.0.16_release.zip (+ .sig + .sha256)  [audit/support]
