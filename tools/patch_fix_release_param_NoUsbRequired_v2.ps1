...
$assignEsc  = [regex]::Escape($assignLine)
$assignLinePattern = "(?m)^\s*$assignEsc\s*\n?"
...
if($open -lt 0){ throw "Parenthèse '(' introuvable après param" }
...
# --- Remove assignment line INSIDE param block (this is the bug)
$paramFixed = [regex]::Replace($paramBlock, $assignLinePattern, "")
