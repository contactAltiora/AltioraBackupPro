...
# ------------------------------------------------------------
# 4) Fix EXISTING --version action='version' (MULTILINE) by inserting:
#      version=VERSION_STR,
#    before the closing ')'
#    IMPORTANT: do NOT add any new --version
# ------------------------------------------------------------
$callPattern = "(?s)\b\w+\.add_argument\(\s*.*?--version.*?action\s*=\s*['""]version['""].*?\)\s*"
...
if($call -match "(?m)^\s*version\s*="){
  ...
  $closePat = "(?m)^(?<ind>\s*)\)\s*$"
  $call2 = [System.Text.RegularExpressions.Regex]::Replace(
    $call,
    $closePat,
    { param($mm)
        $ind = $mm.Groups["ind"].Value
        return ($ind + "    version=VERSION_STR," + "`n" + $ind + ")")
    },
    [System.Text.RegularExpressions.RegexOptions]::Multiline
  )

  if($call2 -eq $call){
    throw "Injection impossible: la ligne de fermeture ')' n'a pas été trouvée dans le bloc --version."
  }
